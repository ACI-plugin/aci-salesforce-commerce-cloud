        ...
        save: function () {
            Transaction.wrap(function () {
                ...
                handlePaymentSelection(cart).error) {// Performs payment method specific checks, such as credit card verification.
                    ...
                    if(paymentMethodID) {
                        var paymentProcessor = PaymentMgr.getPaymentMethod(paymentMethodID).paymentProcessor.ID;
                    }
                    if (Site.getCurrent().getCustomPreferenceValue('ACI_ENABLED')) {

                        var ACIHelper = require('*/cartridge/scripts/aci/aciHelper');
                        if (ACIHelper.isACIPaymentMethod(paymentMethodID)) {
                            // Redirect to stand alone ACI hosted page in case of ACI payments
                            session.privacy.paymentMethodID = paymentMethodID;
                            response.redirect(URLUtils.url('ACI-ShowCheckoutWidget'));
                        } else {
                            // A successful billing page will jump to the next checkout step.
                            app.getController('COSummary').Start();
                        }
                    } else if(paymentProcessor==='ACI' || !paymentMethodID){
                        publicStart();
                    } else {
                        // A successful billing page will jump to the next checkout step.
                        app.getController('COSummary').Start();
